...
public class Guide extends RestAssured {
	...
	@Test
	  public void guide() throws JSONException, IOException, AssertionFailedError{              
		...
		List<ChannelsUtil> chan = gson.fromJson(get("/api/epg/channels").asString(), new TypeToken<List<ChannelsUtil>>(){}.getType());
		...
		List<DatesUtil> dates = gson.fromJson(get("/api/epg/dates").asString(), new TypeToken<List<DatesUtil>>(){}.getType());
		...
		System.out.println("\nValidating the api Guide - for 7 days x 98 channels = 686 scenarios");
		for(String dt: date){
			System.out.println("***** validating for the date ***** "+dt);
